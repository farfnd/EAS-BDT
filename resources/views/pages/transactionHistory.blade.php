<x-layout titlePage="Hanaka | Transaction History">
    ...
    <div class="flex flex-col space-y-5 mx-8 mt-4 mb-6">
        @foreach ($pembayaran as $transaksi)
            @if (count($transaksi->pembayaranDetail) && $transaksi->status_pengiriman != "Beri Ulasan")
                <div class="flex flex-col md:flex-row shadow-custom1 rounded-2xl justify-between px-4 py-3 ">
                    ...
                    <div class="flex">
                        ...
                        <div class="flex flex-col justify-between py-2 ml-3">
                            ...
                            <div>
                                <p>Total Harga: <span class="font-semibold">Rp{{ number_format($transaksi->pembayaranDetail[0]->jumlah_barang * 1000 * $transaksi->pembayaranDetail[0]->barang->harga, 0, ',', '.') }} (Lunas)</span>
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            @elseif($transaksi->status_pengiriman = 'Beri Ulasan')
                ...
                <div class="flex flex-col md:flex-row shadow-custom1 rounded-2xl justify-between px-4 py-3 ">
                            ...
                            <div>
                                ...
                                <p>{{$item->jumlah_barang}}
                                    ...
                                    @elseif ($item->jumlah_barang > 1)
                                        pcs
                                    @endif
                                    @ Rp{{ number_format($item->barang->harga * 1000, 0, ',', '.') }}
                                </p>
                            </div>
                            <div>
                    ...
                    @if (!$item->barang->isReviewed)
                    <div class="flex flex-col justify-between py-2">
                        <button data-micromodal-trigger="beri-ulasan" data-barang_id="{{$item->barang_id}}" data-pembayaran_id="{{$item->pembayaran_id}}" data-tgl="({{ date('d F Y', strtotime($transaksi->created_at)) }})"
                            class="relative w-full bg-gray-800 hover:bg-opacity-90 rounded-lg px-2 py-1 beriUlasan-btn">
                ...
                <div class="modal__container" role="dialog" aria-modal="true" aria-labelledby="beri-ulasan-title"
                    ...
                    <div>
                        ...
                        <main class="modal__content mt-3" id="beri-ulasan-content">
                            <div class="grid grid-cols-12">
                                <div class="col-span-5 flex flex-col space-y-4  mr-4">
                                    <p>Transaksi: <span class="font-bold" id="no_tgl_transaksi">A0012 / 23 Mei 2021</span></p>
                                    <img class="w-40" src="{{ asset('images/jacket.png') }}" alt=""
                                        class="item-image" id="item-image">
                                    <p class="font-bold text-lg" id="item-name">Sweater Logo Fav Maroon</p>
                                </div>
                                <div
                                    class="text-sm md:text-base font-bold col-span-7 flex flex-col justify-center border-l-2 pl-4">
                                    <form id="form-ulasan" enctype="multipart/form-data">
                                        ...
                                        <div class="flex mt-12 space-x-4 justify-end">
                                            ...
                                            <button type="submit"
                                                ...
                                                id="ulasan-submit-btn">Kirim</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
...
<script>
    ...
    $('.beriUlasan-btn').click(function (e) { 
        let button = $(this);
        $('#no_tgl_transaksi').html(button.data('pembayaran_id')+' '+button.data('tgl'));

        $.ajax({
            type: "GET",
            url: "/api/getBarang/" + button.data('barang_id'),
            headers: {
                'Authorization': '{{ session('Authorization') }}'
            },
            async: false,
            success: function(data) {
                $('#item-image').attr('src', `/product_images/${data.foto}`);
                $('#item-name').html(data.nama);
                $('#ulasan-submit-btn').data('id', data.id);
            }
        });
    });
